...
| `components/CreditCard.tsx` | 333 | CreditCard | 4007:57107, 731:33245 | — | components/SerpShell.tsx<br>screens/ViewAll.tsx | Credit-card card — built to the "Mini App Main" spec (Figma `9RfW1gNewOnFDsNqaHsRoF`, frame 4007:57107, component 731:33245), transcribed rather than approxi… |
| `components/ExpandSearch.tsx` | 478 | ExpandSearchCard | 1646:7445 | — | components/SerpShell.tsx | End-of-catalogue AI Expand band — W4 style (Figma 1646:7445), rebuilt on the Aura engine (`motion/Aura.tsx`): a full-bleed lavender band lit from behind by f… |
...
| `components/ResultCards.tsx` | 1032 | StoreRow, ProductCard, StoreTile, DealsCarousel, DealCard, SimilarCardsRail +4 | — | — | components/ExpandSearch.tsx<br>components/SerpShell.tsx<br>screens/CatalogViewAll.tsx<br>screens/ExploreHome.tsx | — |
...
| `components/SerpShell.tsx` | 356 | SerpShell | — | — | screens/Gallery.tsx<br>screens/SearchBody.tsx | SERP shell (§3.3) — the one shape every screen composes: context line → [hero, optional] → content sections → [Expand Search, if any] Hero present only for s… |
...
| Module | LOC | Exports | Figma node(s) | Assets | Used by | Purpose |
| --- | --- | --- | --- | --- | --- | --- |
| `data/catalog.ts` | 428 | STORES, searchStores, buildStoreSerp, PRODUCTS, productItem, searchProducts +11 | — | — | Root.tsx<br>data/productCategories.ts<br>data/realData.ts<br>data/webResults.ts | Store catalog + real search engine. |
...
| `data/realData.ts` | 1156 | BRAND, PRODUCT_IMG, LENDER, ALL_DEALS, cardSbiCashback, cardAxisFlipkart +19 | — | assets/brands<br>assets/products<br>assets/lenders<br>assets/cards<br>assets/banners<br>assets/campaigns | Root.tsx<br>data/catalog.ts<br>data/webResults.ts<br>screens/ExploreHome.tsx | Real CashKaro data — NOTHING here is invented. |
...
| `data/webResults.ts` | 93 | webFeed | — | — | components/ExpandSearch.tsx | Web-results feed for the Expand Search band — an endless, deterministic pager over the REAL product catalog (`./catalog.ts` PRODUCTS; photos, titles, prices… |
...
| `theme/tokens.ts` | 671 | color, fontFamily, letterSpacing, type, space, radius +20 | — | — | Root.tsx<br>components/Badge.tsx<br>components/Button.tsx<br>components/CashbackElement.tsx | CashKaro design tokens — the ONLY source of color, type, spacing, radius, elevation and motion in this app. |
...
| Module | LOC | Exports | Figma node(s) | Assets | Used by | Purpose |
| --- | --- | --- | --- | --- | --- | --- |
| `motion/Aura.tsx` | 571 | AURA_LOOP, useAuraClock, radialFill, softOrbFill, tintRgba, isColourlessTint +8 | — | — | components/ExpandSearch.tsx<br>components/ResultCards.tsx<br>motion/VoiceBlobs.tsx | "Aura" — the flowing-gradient engine behind the AI surfaces (Expand Search). |
...
Modules: 56 · Lines: 13830 · SERP cases: 12
